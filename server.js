...
// Trail geometry lives only here — a writable directory on the same bind-mounted
// /data volume as rides.json, so it persists across redeploys. Deliberately never
// committed to git (see CLAUDE.md "Trail geometry cache") — OSM data is fine to
// redistribute, but TrailLink's isn't, so nothing actually gets checked in;
// instead every environment (local dev, the NAS) is seeded directly via POST.
...
app.get('/api/trail-geometry', (req, res) => {
  const out = {};
  if (!fs.existsSync(TRAIL_GEOM_DIR)) return res.json(out);
