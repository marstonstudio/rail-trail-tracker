...
**Map legend vs. trail popups**: the bottom-left `.map-legend` box (Ridden/Nearby trail/Parking key) and a trail's info popup both anchor to the same bottom-left corner and visually overlap, so the legend hides (`legend-hidden` class) whenever any popup is open, via `map.on('popupopen'/'popupclose', ...)` — global listeners on the map, not per-marker, so this covers every trail card (Nearby cards, ridden rides, map markers) automatically without each call site needing its own show/hide logic.
