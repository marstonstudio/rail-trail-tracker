...
What replaced it: any Nearby card for a trail with no cached geometry (`!(TRAIL_POLYLINES[id]?.length > 0)`) shows a small "📋 Investigate" button (`copyTrailInvestigatePrompt(id)`). It copies a ready-to-paste Claude Code prompt — trail name, id, coordinates, any existing `osmNames` aliases, and source URL — framed as an investigation task pointing at `fetchTrailPolylineDirectAttempt()`, with an explicit reminder to commit the fix into `trail-geometry-seed/<id>.json` once verified rather than leaving it only in the local runtime cache. The button disappears once that trail has real geometry.

The background fetch loop (`fetchAllNearbyTrailLines`) and the not-found retry cap (`MAX_AUTO_RETRIES`, `autoRetryExhausted()`) both still exist quietly, with no visible UI — they're just the fallback that tries to fill in geometry for any trail not yet in `trail-geometry-seed/`, without hammering Overpass.
