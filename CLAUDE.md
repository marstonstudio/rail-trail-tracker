...
trail-geometry-seed/               Committed, verified trail-path data — see Trail geometry cache
...
**Two-tier storage — committed seed + writable runtime cache**: once a trail's geometry has been fetched and verified clean, it's checked into the repo at `trail-geometry-seed/<id>.json` and bundled into the Docker image (`COPY trail-geometry-seed ./trail-geometry-seed` in the Dockerfile) — this is the durable source of truth, so a known-good trail keeps working even if Overpass is down, rate-limited, or the whole matching pipeline changes again. The writable runtime cache at `<data-dir>/trail-geometry/<id>.json` (same bind-mounted `/data` volume as `rides.json`) is only for trails not yet promoted into the seed, or a fresher fetch not yet committed — `GET /api/trail-geometry` merges both (`readGeomDir()` in `server.js`), with the runtime cache winning on conflict. In practice this means: Overpass is a tool used to *populate or fix* the seed (by me, during development), never something the live app depends on to show a trail that's already known good. When you fetch/fix a trail's geometry, after verifying it's clean, copy it from the runtime cache into `trail-geometry-seed/` (bump its `version` field to match `TRAIL_GEOM_LOGIC_VERSION` first) and commit it — don't leave a corrected trail sitting only in the gitignored runtime `data/` dir, or it'll only exist on whichever single environment you happened to seed it on.

**Cache versioning / cache-busting**: `TRAIL_GEOM_LOGIC_VERSION` (in `index.html`, currently `6`) must be bumped whenever the trail-matching logic *or* a specific trail's `osmNames`/`lat`/`lng` changes in a way that could change which OSM geometry gets attributed to a trail — e.g. editing `namesMatch()`, the distance cap/tiebreak, or adding aliases for a trail whose real OSM name differs from its declared name. `loadTrailGeometryCache()` only hydrates entries whose stored `version` matches the current constant; anything older (or in the pre-versioning legacy plain-array format) is silently treated as uncached and gets naturally re-fetched with current logic. **A version bump invalidates the committed seed too** — every seed file's `version` field needs bumping alongside the constant (a quick script, not a re-fetch, since the underlying geometry hasn't changed) or every trail looks stale again and the app falls back to live Overpass fetching for all of them until each is manually re-promoted. Also remember to clear any affected trail's entry from `data/trail-fetch-failures.json` if it had previously exhausted its not-found retry cap — a version bump alone doesn't reset that separate persisted store, so a trail fixed by adding an alias can still stay artificially capped.

**v5 note**: bumping past a manually-curated entry's stamped version (e.g. `n109` Wompatuck, `n111` Yankee Doodle, `n112` Nickerson) makes it look stale too, even though it deliberately bypasses the matching pipeline — re-`POST` those entries (and re-commit them into `trail-geometry-seed/`) with the new version number alongside any version bump, or they'll silently drop out of the served set until re-seeded.

On page load, `loadTrailGeometryCache()` bulk-fetches everything already cached (seed + runtime merge) in one request and hydrates `TRAIL_POLYLINES` before the first render — **there is no background fetch loop anymore**. A trail with no seeded geometry just shows no line; see "Per-trail investigation prompt" below for how that gets fixed. The runtime `<data-dir>/trail-geometry/` directory lives under the same `/data` directory as `rides.json` and `fits/`, so it's covered by the existing bind-mount volume and survives container redeploys with no extra Docker config — but it's a fallback for the not-yet-seeded case, not the primary source anymore.
...
What replaced it: any Nearby card *and* map popup for a trail with no cached geometry (`!(TRAIL_POLYLINES[id]?.length > 0)`) shows a small "📋 Investigate" button (`copyTrailInvestigatePrompt(id)`, ids `investigate-<id>` on the card / `investigate-popup-<id>` in the popup — `copyTrailInvestigatePrompt` checks both so either caller gets its checkmark). It copies a ready-to-paste Claude Code prompt — trail name, id, coordinates, any existing `osmNames` aliases, and source URL — framed as an investigation task pointing at `fetchTrailPolylineDirectAttempt()` (still used by the ride-import match-picker, see below), with an explicit reminder to commit the fix into `trail-geometry-seed/<id>.json` once verified rather than leaving it only in the local runtime cache. The button disappears once that trail has real geometry.
