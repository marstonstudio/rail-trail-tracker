...
- **Rate-limit banner**: whenever any trail is currently `rate-limited`, an inline banner explains that Overpass rate-limited these trails and each already got its one automatic internal retry (see `fetchTrailPolylineDirect()` — there's no continuous auto-retry loop beyond that single retry per page load, so wording here must not imply otherwise). The banner also shows live specifics fetched from Overpass's own `https://overpass-api.de/api/status` endpoint (`fetchOverpassStatus()`, 5s cache) — parses `Rate limit: N`, `N slots available now.`, and any `Slot available after: ..., in N seconds.` lines to report free slot count and next-slot wait time (`updateRateLimitBannerDetail()`, filled in async after the panel's initial paint so it doesn't block rendering). Each rate-limited row also shows how many seconds ago its last attempt happened, from `trailFetchStatus[id].updatedAt`.
