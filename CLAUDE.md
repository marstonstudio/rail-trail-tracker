...
**Heading-up rotation**: Ride Mode always rotates the map so the rider's direction of travel points to the top of the screen — matches a phone mounted sideways on the bike (notch-left landscape). `onRideHeadingSample()` uses `pos.coords.heading` when available, falling back to the bearing between consecutive fixes (`bearingDeg()`, only once they're >3m apart, to avoid GPS-noise jitter) when stationary/unsupported, then low-pass-smooths it (`angleDelta()`) so the rotation eases rather than snaps. `updateRideHeadingRotation()` applies `rotate(${-heading}deg)` to both `#map` and the compass needle. Since Leaflet has no native rotation, `#map` is enlarged via JS (`applyRideMapRotationLayout()`) to cover the viewport's diagonal and centered with negative margins, so a rotated corner never exposes blank space — `resetRideMapLayout()` restores the normal 100%-fill layout on exit. Leaflet's `.leaflet-control-container` (zoom buttons) is reparented from `#map` onto the sibling `.map-wrap` once at map init specifically so it doesn't get dragged along with `#map`'s rotation. A 🧭 compass badge (top-left, `#ride-compass`) shows a needle that rotates the same amount (it points at true north, a fixed direction on the rotated map) plus a live cardinal-direction label (N/NE/E/etc).
